function! myspacevim#before() abort
   ...
   set autoindent
    ...
    nmap <C-s> :<C-U>w<CR><Plug>JupyterExecute
...
function! myspacevim#gme_rename() abort
  %s'^#pragma'// #pragma'ge
  %s'// #pragma once'#pragma once'ge
  %s/@defgroup \([^G]\)/@defgroup GME\1/ge
  %s/^gme_//ge
  %s/ACIS_NEW/new/ge
  %s/ACIS_DELETE/delete/ge
  %s/mid_point/gme_mid_point/ge
  %s/"gme",//ge
  %s/operator_add(\(.*\), \(.*\))/(\1) + (\2)/ge
  %s/operator_sub(\(.*\), \(.*\))/(\1) - (\2)/ge
  %s/operator_sub(\(.*\))/- (\1)/ge
  %s/new /GME_NEW /ge
  %s/delete /GME_DELETE /ge
endfunction
